...
public class ProductControllerWebMvcTest {
    ...
    void updateProductShouldReturnUpdatedProduct() throws Exception {
        GenericProductDto productToUpdate = new GenericProductDto();
        productToUpdate.setId(1001L);
        productToUpdate.setTitle("iPhone 15 Pro Max");
        productToUpdate.setImage("iPhone image");
        productToUpdate.setCategory("mobile phones");
        productToUpdate.setDescription("Best iPhone Ever");

        GenericProductDto expectedProduct = new GenericProductDto();
        expectedProduct.setId(1001L);
        expectedProduct.setTitle("iPhone 15 Pro Max");
        expectedProduct.setImage("iPhone image");
        expectedProduct.setCategory("mobile phones");
        expectedProduct.setDescription("Best iPhone Ever");

        when(
                productService.updateProduct(any(), any())
        ).thenReturn(
                expectedProduct
        );
